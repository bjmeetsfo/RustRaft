...
| Native ByteRaft kvbench bridge | The public ByteRaft tree includes `example/kv/kv_benchmark.cc` and `script/bench.sh`, but that benchmark is client-workload oriented and does not cover the whole parity matrix. | Use `rustraft_probe_byteraft_native_benchmark()` to report available `kvbench` evidence, then add or provide a full JSON `byteraft_parity_benchmark` harness for all required workloads. | Native `kvbench` is reported as partial evidence and production parity remains blocked until the full harness exists. |
| Executable native ByteRaft adapter | Native `kvbench` evidence previously stopped before producing a parity report. | Use `scripts/byteraft_native_kvbench_adapter.sh` through `--native-kvbench-adapter` to launch the ByteRaft KV example for supported workloads and emit fail-closed JSON samples for unsupported workloads. | The adapter can produce a normal benchmark report, but unsupported WAL, snapshot, and leader-transfer workloads keep production parity blocked. |
